...
class ControllerExtensionModuleAllreviews extends Controller {
	...
	$check = $this->db->query("SELECT * FROM `" . DB_PREFIX . "setting` where `code`='module_allreviews'");
		if ($check->num_rows < 1){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `code`='module_allreviews', `key`='module_allreviews_status', `value`='1'");
		}	
