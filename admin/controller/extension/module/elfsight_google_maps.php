...
class ControllerExtensionModuleElfsightGoogleMaps extends Controller {
	private $error = array();

	public $params = NULL;
    
    const ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/google-maps/?utm_source=portals&utm_medium=opencart&utm_campaign=google-maps&utm_content=sign-up';
	...
	public function index() {
		$this->load->language('extension/module/elfsight_google_maps');
		...
		$data['action'] = $this->url->link('extension/module/elfsight_google_maps', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/elfsight_google_maps', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/elfsight_google_maps')) {
