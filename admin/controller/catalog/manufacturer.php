...
class ControllerCatalogManufacturer extends Controller {
	...
	public function index() {
		...
		$this->document->setTitle('Manufacturers');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_filter->addManufacturer($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_filter->editManufacturer($this->request->get['filter_group_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $filter_group_id) {
				$this->model_catalog_filter->deleteManufacturer($filter_group_id);
			}
	...
	protected function getList() {
		...
		$results = $this->model_catalog_filter->getFilterDescriptions('1', $filter_data);
		$filter_total = $this->model_catalog_filter->getTotalFiltersByGroup('1');
		foreach ($results as $result) {
			$data['filters'][] = array(
				'filter_group_id' => $result['filter_id'],
				'name'            => isset($result['filter_description'][1]['name']) ? $result['filter_description'][1]['name'] : $result['filter_description'][1]['name'],
				'sort_order'      => $result['sort_order'],
				'edit'            => $this->url->link('catalog/manufacturer/edit', 'user_token=' . $this->session->data['user_token'] . '&filter_group_id=' . $result['filter_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/manufacturer', 'user_token=' . $this->session->data['user_token'] . '&sort=fgd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/manufacturer', 'user_token=' . $this->session->data['user_token'] . '&sort=fg.sort_order' . $url, true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['filter_group_id'])) {
			...
			$data['action'] = $this->url->link('catalog/manufacturer/edit', 'user_token=' . $this->session->data['user_token'] . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true);
		}
