...
class ModelCatalogFilter extends Model {
	...
	public function getFilters($data) {
		...
		if (!empty($data['filter_group'])) {
			$sql .= " AND f.filter_group_id = '".$data['filter_group']."'";
		}
	...
	public function getFilterDescriptions($filter_group_id, $data=array()) {
		$filter_data = array();
		$sql = "SELECT * FROM " . DB_PREFIX . "filter WHERE filter_group_id = '" . (int)$filter_group_id . "'";
		if (isset($data['start']) || isset($data['limit'])) {
			if ($data['start'] < 0) {
				$data['start'] = 0;
			}
	...
	public function getTotalFilters() {
		$query = $this->db->query("SELECT COUNT(case filter_group_id when '2' then 1 else null end) AS total FROM `" . DB_PREFIX . "filter`");

		return $query->row['total'];
	}

	public function getTotalFiltersByGroup($group_id) {
		$query = $this->db->query("SELECT COUNT(case filter_group_id when '".$group_id."' then 1 else null end) AS total FROM `" . DB_PREFIX . "filter`");
...
//////////////////////////By Mahmoud
    public function editModel($filter_id, $data) {
        $this->db->query("UPDATE `" . DB_PREFIX . "filter` SET sort_order = '" . (int)$data['sort_order'] . "', parent_id = '".(int)$data['parent_id']. "' WHERE filter_id = '" . (int)$filter_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "filter_description WHERE filter_id = '" . (int)$filter_id . "'");
        
        foreach ($data['filter_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', filter_group_id = '2', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
		}
    }

    public function addModel($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "filter` SET sort_order = '" . (int)$data['sort_order'] . "', filter_group_id = '2'" . ", parent_id = '".(int)$data['parent_id']."'");
    ...
    public function deleteModel($filter_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter` WHERE filter_id = '" . (int)$filter_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_description` WHERE filter_id = '" . (int)$filter_id . "'");
    }
    
    ////////////////////////////////////////////////////
    public function editManufacturer($filter_id, $data) {
        $this->db->query("UPDATE `" . DB_PREFIX . "filter` SET sort_order = '" . (int)$data['sort_order'] . "', parent_id = '".(int)$data['parent_id']. "' WHERE filter_id = '" . (int)$filter_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "filter_description WHERE filter_id = '" . (int)$filter_id . "'");
        
        foreach ($data['filter_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', filter_group_id = '1', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
		}
    }

    public function addManufacturer($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "filter` SET sort_order = '" . (int)$data['sort_order'] . "', filter_group_id = '1'" . ", parent_id = '".(int)$data['parent_id']."'");
    ...
    public function deleteManufacturer($filter_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter` WHERE filter_id = '" . (int)$filter_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_description` WHERE filter_id = '" . (int)$filter_id . "'");
	}
	...
	public function getModelDescriptions($filter_group_id, $data=array()) {
		$filter_group_data = array();

		$query = $this->db->query("SELECT fd.language_id, fd.name, f.filter_id, f.parent_id FROM " . DB_PREFIX . "filter_description fd LEFT JOIN " . DB_PREFIX . "filter f ON (f.filter_id = fd.filter_id) WHERE fd.filter_id = '" . (int)$filter_group_id . "'");

		foreach ($query->rows as $result) {
			$filter_group_data[$result['language_id']] = array('name' => $result['name'], 'parent_id'=>((int)$result['parent_id']));
		}
		return $filter_group_data;
	}
}
